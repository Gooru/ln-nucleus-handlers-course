...
public class MoveLessonToUnitHandler implements DBHandler {
  ...
  public ExecutionResult<MessageResponse> checkSanity() {
    ...
    if (context.request() == null || context.request().isEmpty()) {
      ...
      return new ExecutionResult<>(
          MessageResponseFactory
              .createInvalidRequestResponse("Invalid data provided to move lesson"),
          ExecutionStatus.FAILED);
    }
    ...
    if (missingFieldErrors != null && !missingFieldErrors.isEmpty()) {
      return new ExecutionResult<>(
          MessageResponseFactory.createValidationErrorResponse(missingFieldErrors),
  ...
  public ExecutionResult<MessageResponse> validateRequest() {
    ...
    LazyList<AJEntityCourse> targetCourses = AJEntityCourse
        .findBySQL(AJEntityCourse.SELECT_COURSE_TO_AUTHORIZE, targetCourseId, false,
            context.userId(),
            context.userId());
    LazyList<AJEntityCourse> sourceCourses = AJEntityCourse
        .findBySQL(AJEntityCourse.SELECT_COURSE_TO_AUTHORIZE, sourceCourseId, false,
            context.userId(),
            context.userId());

    if (targetCourses.isEmpty() || sourceCourses.isEmpty()) {
      LOGGER.warn(
          "user is not owner or collaborator of source or target course to move lesson. aborting");
      return new ExecutionResult<>(MessageResponseFactory.createForbiddenResponse(),
          ExecutionStatus.FAILED);
    }
    ...
    LazyList<AJEntityLesson> lessons = AJEntityLesson
        .findBySQL(AJEntityLesson.SELECT_LESSON_TO_VALIDATE, lessonToMove, sourceUnitId,
  ...
  public ExecutionResult<MessageResponse> executeRequest() {
    ...
    Object maxSequenceId =
        Base.firstCell(AJEntityLesson.SELECT_LESSON_MAX_SEQUENCEID, context.courseId(),
            context.unitId());
    int sequenceId = 1;
    ...
    if (lessonToUpdate.save()) {
      ...
      AJEntityCollection.update(
          ...
          "lesson_id = ?::uuid", context.courseId(), context.unitId(), targetCourseOwner,
          context.userId(), null,
          lessonToUpdate.getId());
      AJEntityContent
          .update("course_id = ?::uuid, unit_id = ?::uuid, modifier_id = ?::uuid",
              "lesson_id = ?::uuid",
              context.courseId(), context.unitId(), context.userId(), lessonToUpdate.getId());

      return new ExecutionResult<>(
          MessageResponseFactory.createNoContentResponse(EventBuilderFactory
              .getMoveLessonEventBuilder(context.courseId(), context.unitId(),
                  lessonToUpdate.getId().toString(),
                  context.request())), ExecutionStatus.SUCCESSFUL);
    } else {
  ...
  private JsonObject validateNullFields() {
    ...
    input.fieldNames().stream().filter(
        key -> AJEntityUnit.LESSON_MOVE_NOTNULL_FIELDS.contains(key) && (input.getValue(key) == null
            || input
  ...
  private JsonObject getModelErrors() {
    ...
    this.lessonToUpdate.errors().entrySet()
