...
public class CreateUnitHandler implements DBHandler {
    ...
    public ExecutionResult<MessageResponse> checkSanity() {
        ...
        if (context.request() == null || context.request().isEmpty()) {
            ...
            return new ExecutionResult<>(
                MessageResponseFactory.createInvalidRequestResponse("Invalid data provided to create unit"),
                ExecutionStatus.FAILED);
        }
    ...
    public ExecutionResult<MessageResponse> executeRequest() {
        ...
        if (newUnit.isValid()) {
            if (newUnit.save()) {
                LOGGER.info("unit {} created successfully for course {}", newUnit.getId().toString(),
                    context.courseId());
                ...
                return new ExecutionResult<>(
                    MessageResponseFactory.createPostResponse(newUnit.getId().toString(),
                        EventBuilderFactory.getCreateUnitEventBuilder(newUnit.getId().toString())),
    ...
    private JsonObject validateFields() {
        ...
        input.fieldNames().stream().filter(key -> !AJEntityUnit.INSERTABLE_FIELDS.contains(key))
