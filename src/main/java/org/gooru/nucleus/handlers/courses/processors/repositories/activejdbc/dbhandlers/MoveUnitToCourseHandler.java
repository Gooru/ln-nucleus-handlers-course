...
public class MoveUnitToCourseHandler implements DBHandler {
  ...
  public ExecutionResult<MessageResponse> validateRequest() {
    ...
    if (sourceCourseId == null || unitToMove == null || sourceCourseId.isEmpty() || unitToMove
        .isEmpty()) {
      LOGGER.warn("missing required fields");
      return new ExecutionResult<>(MessageResponseFactory
          .createValidationErrorResponse(
              new JsonObject().put("message", "missing required fields")),
          ExecutionResult.ExecutionStatus.FAILED);
    }
    ...
    if (targetCourses.isEmpty() || sourceCourses.isEmpty()) {
      LOGGER.warn(
          "user is not owner or collaborator of source or target course to move unit. aborting");
      return new ExecutionResult<>(MessageResponseFactory.createForbiddenResponse(),
          ExecutionStatus.FAILED);
    }
    ...
    LazyList<AJEntityUnit> units =
        AJEntityUnit
            .findBySQL(AJEntityUnit.SELECT_UNIT_TO_VALIDATE, unitToMove, sourceCourseId, false);
    if (units.isEmpty()) {
  ...
  public ExecutionResult<MessageResponse> executeRequest() {
    ...
    if (unitToUpdate.save()) {
      ...
      AJEntityLesson.update("course_id = ?::uuid, owner_id = ?::uuid, modifier_id = ?::uuid",
          "unit_id = ?::uuid",
          context.courseId(), targetCourseOwner, context.userId(), unitToUpdate.getId());
      AJEntityCollection
          .update(
              "course_id = ?::uuid, owner_id = ?::uuid, modifier_id = ?::uuid, collaborator = ?",
              "unit_id = ?::uuid", context.courseId(), targetCourseOwner, context.userId(), null,
              unitToUpdate.getId());
      AJEntityContent
          .update("course_id = ?::uuid, modifier_id = ?::uuid", "unit_id = ?::uuid",
              context.courseId(),
              context.userId(), unitToUpdate.getId());

      return new ExecutionResult<>(
          MessageResponseFactory.createNoContentResponse(EventBuilderFactory
              .getMoveUnitEventBuilder(context.courseId(), unitToUpdate.getId().toString(),
                  context.request())),
  ...
  private JsonObject validateNullFields() {
    ...
    input.fieldNames().stream().filter(
        key -> AJEntityCourse.UNIT_MOVE_NOTNULL_FIELDS.contains(key) && (input.getValue(key) == null
            || input
