...
public class FetchCourseCardsHandler implements DBHandler {
    ...
    public ExecutionResult<MessageResponse> checkSanity() {
        JsonArray inputArray = context.request().getJsonArray("courseids");
        searchValue = inputArray != null ? inputArray.getString(0) : null;

        if (searchValue == null || searchValue.isEmpty()) {
            LOGGER.warn("Invalid list of courses");
            return new ExecutionResult<>(MessageResponseFactory.createInvalidRequestResponse("Invalid course cards request"),
