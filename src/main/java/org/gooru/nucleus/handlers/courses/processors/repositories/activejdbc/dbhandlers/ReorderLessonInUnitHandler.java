...
public class ReorderLessonInUnitHandler implements DBHandler {
    ...
    public ExecutionResult<MessageResponse> checkSanity() {
        if (context.courseId() == null || context.courseId().isEmpty()) {
            ...
            return new ExecutionResult<>(
                MessageResponseFactory.createInvalidRequestResponse("Invalid course id provided to reorder lessons"),
        ...
        if (context.unitId() == null || context.unitId().isEmpty()) {
            ...
            return new ExecutionResult<>(
                MessageResponseFactory.createInvalidRequestResponse("Invalid unit id provided to reorder lessons"),
                ExecutionStatus.FAILED);
        }
        ...
        if (context.request() == null || context.request().isEmpty()) {
            ...
            return new ExecutionResult<>(
                MessageResponseFactory.createInvalidRequestResponse("Invalid data provided to reorder lessons"),
                ExecutionStatus.FAILED);
        }
    ...
    public ExecutionResult<MessageResponse> executeRequest() {
        try {
            List lessonsOfUnit =
                Base.firstColumn(AJEntityLesson.SELECT_LESSON_OF_COURSE, context.unitId(), context.courseId(), false);
            JsonArray input = this.context.request().getJsonArray(REORDER_PAYLOAD_KEY);

            if (lessonsOfUnit.size() != input.size()) {
                return new ExecutionResult<>(
                    MessageResponseFactory.createInvalidRequestResponse("Lesson count mismatch"),
            ...
            for (Object entry : input) {
                ...
                Base.addBatch(ps, sequenceId, this.context.userId(), payloadLessonId, context.unitId(),
        ...
        return new ExecutionResult<>(MessageResponseFactory.createNoContentResponse(
            EventBuilderFactory.getReorderLessonEventBuilder(context.unitId())), ExecutionStatus.SUCCESSFUL);
    }
