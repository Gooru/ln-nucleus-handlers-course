...
public class UpdateUnitHandler implements DBHandler {
	...
	public ExecutionResult<MessageResponse> checkSanity() {
		if (context.courseId() == null || context.courseId().isEmpty()) {
			...
			return new ExecutionResult<>(
					MessageResponseFactory.createInvalidRequestResponse("Invalid course id provided to update unit"),
		...
		if (context.unitId() == null || context.unitId().isEmpty()) {
			...
			return new ExecutionResult<>(
					MessageResponseFactory.createInvalidRequestResponse("Invalid unit id provided to update unit"),
					ExecutionStatus.FAILED);
		}
		...
		if (context.request() == null || context.request().isEmpty()) {
			...
			return new ExecutionResult<>(
					MessageResponseFactory.createInvalidRequestResponse("Invalid data provided to update unit"),
					ExecutionStatus.FAILED);
		}
	...
	public ExecutionResult<MessageResponse> validateRequest() {
		...
		LazyList<AJEntityUnit> ajEntityUnit = AJEntityUnit.findBySQL(AJEntityUnit.SELECT_UNIT_TO_VALIDATE,
	...
	private JsonObject validateFields() {
		...
		input.fieldNames().stream().filter(key -> !AJEntityUnit.UPDATABLE_FIELDS.contains(key))
	...
	private JsonObject validateNullFields() {
		...
		input.fieldNames().stream()
				.filter(key -> AJEntityUnit.NOTNULL_FIELDS.contains(key)
