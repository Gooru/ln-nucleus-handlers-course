...
public class FetchCourseHandler implements DBHandler {
  ...
  private JsonArray getLessonCountsForUnitsInCourse(LazyList<AJEntityUnit> units) {
    ...
    List<Map> lessonCounts = Base
        .findAll(AJEntityLesson.SELECT_LESSON_COUNT_MULTIPLE, context.courseId());
    ...
    units.forEach(unit -> {
      JsonObject unitSummary = new JsonObject(
          new JsonFormatterBuilder()
              .buildSimpleJsonFormatter(false, AJEntityUnit.UNIT_SUMMARY_FIELDS)
              .toJson(unit));
