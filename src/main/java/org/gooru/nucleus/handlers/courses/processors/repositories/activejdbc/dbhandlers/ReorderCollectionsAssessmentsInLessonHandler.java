...
public class ReorderCollectionsAssessmentsInLessonHandler implements DBHandler {
  ...
  private static final Logger LOGGER = LoggerFactory
      .getLogger(ReorderCollectionsAssessmentsInLessonHandler.class);
  ...
  public ExecutionResult<MessageResponse> checkSanity() {
    if (context.courseId() == null || context.courseId().isEmpty()) {
      ...
      return new ExecutionResult<>(MessageResponseFactory
          .createInvalidRequestResponse("Invalid course id provided to reorder lesson contents"),
    ...
    if (context.unitId() == null || context.unitId().isEmpty()) {
      ...
      return new ExecutionResult<>(MessageResponseFactory
          .createInvalidRequestResponse("Invalid unit id provided to reorder lesson contents"),
          ExecutionStatus.FAILED);
    }
    ...
    if (context.request() == null || context.request().isEmpty()) {
      ...
      return new ExecutionResult<>(
          MessageResponseFactory
              .createInvalidRequestResponse("Invalid data provided to reorder lesson contents"),
          ExecutionStatus.FAILED);
    }
  ...
  public ExecutionResult<MessageResponse> validateRequest() {
    ...
    if (ajEntityCourse.isEmpty()) {
      LOGGER
          .warn("user is not owner or collaborator of course to reorder lesson content. aborting");
      return new ExecutionResult<>(MessageResponseFactory.createForbiddenResponse(),
          ExecutionStatus.FAILED);
    }
  ...
  public ExecutionResult<MessageResponse> executeRequest() {
    try {
      ...
      List contentOfLesson =
          Base.firstColumn(AJEntityCollection.SELECT_COLLECTION_OF_COURSE, context.lessonId(),
              context.unitId(),
              context.courseId(), false);
      ...
      if (contentOfLesson.size() != input.size()) {
        return new ExecutionResult<>(
            MessageResponseFactory
                .createInvalidRequestResponse("Collection/Assessment count mismatch"),
      ...
      for (Object entry : input) {
        ...
        if (!contentOfLesson.contains(UUID.fromString(payloadContentId))) {
          return new ExecutionResult<>(
              MessageResponseFactory
                  .createInvalidRequestResponse("Missing collection(s)/assessment(s)"),
              ExecutionResult.ExecutionStatus.FAILED);
        }
    ...
    return new ExecutionResult<>(MessageResponseFactory
        .createNoContentResponse(
            EventBuilderFactory.getReorderCollectionEventBuilder(context.lessonId())),
