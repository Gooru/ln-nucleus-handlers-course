...
public final class TagAggregatorRequestBuilderFactory {
  ...
  public static TagAggregatorRequestBuilder getUnitTagAggregatorRequestBuilder(String unitId,
      JsonObject tags) {
    return () -> new JsonObject().put(ENTITY_ID, unitId).put(ENTITY_TYPE, ENTITY_UNIT)
        .mergeIn(tags);
  }

  public static TagAggregatorRequestBuilder getCourseTagAggregatorRequestBuilder(String courseId,
      JsonObject tags) {
    return () -> new JsonObject().put(ENTITY_ID, courseId).put(ENTITY_TYPE, ENTITY_COURSE)
        .mergeIn(tags);
  }
}
